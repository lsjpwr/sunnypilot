...
`update_location`은 켜진 종류 집합을 `next_camera(..., kinds=...)`에 넘기고, 매 틱 두 번 조회한다. 하나는 표지용(`self.camera`, 원뿔)이고, 다른 하나는 감속용(`self.slowdown_camera`, `corridor_m=CAMERA_CORRIDOR_M`)이다.
...
- 진행선에서 옆으로 30 m보다 먼 카메라(옆 도로·반대 차로)는 HUD 앞쪽 표지에만 나오고 감속하지 않는다. 그래서 표지(원뿔)와 감속(회랑)이 서로 다른 카메라를 가리킬 수 있다 — 옆 골목 카메라 때문에 "AHEAD 30"이 떠도 차는 줄지 않는다. 곡선 구간 카메라 일부는 늦게 감속하거나 표지만 뜬다(2절). 굽은 도심 고속화도로에서는 옆 도로 카메라가 SCC-Map 감속 거리 안에서 잠깐 잡혀 짧게 감속할 수 있다(재리뷰 실데이터: 동부간선도로에서 2대).
...
- **카메라를 차가 달리는 링크에 매칭** — 회랑 검사 대신 차의 현재 링크와 카메라를 맞추면 곡선 구간 카메라도 감속 대상으로 되살릴 수 있다. 별도 작업으로 한다.
