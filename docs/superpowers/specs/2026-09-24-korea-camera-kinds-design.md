...
```python
def next_camera(self, lat, lon, heading_deg, route=None, kinds=None, corridor_m=None) -> Camera | None:
```

`kinds`가 주어지면 그 집합에 없는 종류는 거리 비교 전에 건너뛴다. 그래서 가까운 꺼진 카메라가 더 먼 켜진 카메라를 가리지 않는다. `kinds=None`은 전부 허용한다(기존 호출·테스트와 호환). 종류를 모르는 카메라(구 DB)는 비어 있지 않은 어떤 집합이든 통과한다. 넷을 모두 끄면(빈 집합) 구 DB에서도 카메라가 없는 것과 같다. 키가 없어 DB가 영영 재생성되지 않는 기기에서도 이것으로 카메라 감속을 끌 수 있다(최종 리뷰 I2).

`corridor_m`이 주어지면 진행선에서 옆으로 그보다 먼 카메라도 뺀다(`_on_path`의 회랑 검사, 방지턱과 같은 방식). HUD 표지용 조회는 이 값을 주지 않아 지금처럼 ±60° 원뿔로 고른다. 감속용 조회는 `CAMERA_CORRIDOR_M = 30`을 준다. 경로가 없으면(기본) 원뿔만으로는 옆 골목·평행 도로·반대 차로의 카메라가 자주 잡힌다. 최종 리뷰가 실데이터로 올림픽대로·강변북로·동부간선도로를 달려 보니, 원뿔만 쓸 때 도로 밖 30~50 km/h 카메라가 한 번 주행에 수십 번 감속 대상이 됐다. 회랑을 쓰면 올림픽대로·강변북로에서 0곳, 동부간선도로에서 4곳으로 줄었다. 대가로 곡선 구간 카메라 일부는 늦게 감속한다. 회랑에 들어오는 시점이 SCC-Map의 감속 거리보다 가까우면 플래너의 평소 감속도(−1.2 m/s²)로 줄이다가 제한속도를 넘긴 채 카메라를 지난다. 아예 표지만 뜨고 감속하지 않는 경우도 있다. 재리뷰 실데이터에서는 같은 방향 본선 카메라 6대 중 1대가 늦었고 1대는 감속하지 않았다(최종 리뷰 C1).

**경로 안내 중에는 경로선이 회랑을 대신한다(2026-09-25 추가).** 차가 경로선에서 `ROUTE_CORRIDOR_M`(30 m) 안에 있으면 감속용 조회에 진행선 회랑을 주지 않고 표지용 카메라를 그대로 쓴다. `next_camera`가 경로가 있으면 이미 경로선 30 m 안의 카메라만 받고, 경로선은 도로를 따라 굽으므로 곡선 구간 카메라도 제때 감속한다. 진행선 회랑을 같이 걸면 곧은 길에서는 결과가 같고 굽은 길에서는 경로 위 카메라만 빠진다. 차가 경로선에서 30 m 넘게 벗어나면 진행선 회랑을 다시 쓴다. `RouteSource`는 재탐색이 성공할 때까지 옛 경로를 들고 있어서(신호가 없으면 몇 분), 그동안 경로선만 믿으면 떠난 도로의 카메라에 감속한다.
...
`update_location`은 켜진 종류 집합을 `next_camera(..., kinds=...)`에 넘기고, 매 틱 두 번 조회한다. 하나는 표지용(`self.camera`, 원뿔)이고, 다른 하나는 감속용(`self.slowdown_camera`, `corridor_m=CAMERA_CORRIDOR_M`)이다. 차가 경로선 30 m 안에 있으면 감속용은 조회하지 않고 표지용 카메라를 그대로 쓴다(2절).
...
**감속:** `publish_targets`(`korea_map_data.py:253`)가 감속용 카메라(`slowdown_camera`)의 점을 방지턱·커브 점과 같은 리스트에 넣는다. 조건은 방지턱과 같다 — `localizer_valid`이고 위치가 있을 때만.

- 점 위치: 차에서 카메라로 가는 직선 위, 차에서 `거리 − 여유`만큼 떨어진 곳. 위경도 선형 보간이면 충분하다(2 km 이내).
- 점은 카메라마다 한 번만 정하고 그 자리에 고정한다. 키는 (카메라 위경도, 여유 거리)다. SCC-Map은 위도·경도·속도가 똑같은 점이 리스트에 남아 있을 때만 목표를 유지한다(`map_controller.py:208-223`). 매 틱 차 위치에서 다시 계산하면 점이 조금씩 움직여 목표를 놓았다 잡았다 한다(최종 리뷰 I1). 카메라가 후보에서 빠지면 고정점도 버린다. 그래서 같은 카메라를 반대 방향에서 다시 만나면 새로 정한다.
- 이미 여유 구간 안이면(거리 ≤ 여유) 점은 차 위치다. SCC-Map은 거리 0짜리 점을 계속 유효로 보므로 카메라를 지날 때까지 제한속도 부근을 유지한다. 정확히는 SCC-Map이 `v_ego`가 목표 아래로 내려가면 점을 놓고(`map_controller.py:161`) 다시 넘으면 잡는다 — 방지턱·커브와 같은 동작이고, 속도는 목표 근처에서 머문다. 감속용 카메라가 후보에서 빠지는 순간 점과 고정점도 함께 사라진다. 진행방향 ±60°를 벗어나거나 진행선에서 30 m 밖으로 나가면 후보에서 빠진다. 감속이 시작된 뒤 회랑에서 한 번 빠지면 SCC-Map도 목표를 놓는다.
...
- 진행선에서 옆으로 30 m보다 먼 카메라(옆 도로·반대 차로)는 HUD 앞쪽 표지에만 나오고 감속하지 않는다. 그래서 표지(원뿔)와 감속(회랑)이 서로 다른 카메라를 가리킬 수 있다 — 옆 골목 카메라 때문에 "AHEAD 30"이 떠도 차는 줄지 않는다. 곡선 구간 카메라 일부는 늦게 감속하거나 표지만 뜬다(2절). 경로 안내 중 경로 위에서는 진행선 대신 경로선으로 걸러서 이 한계가 없다. 굽은 도심 고속화도로에서는 옆 도로 카메라가 SCC-Map 감속 거리 안에서 잠깐 잡혀 짧게 감속할 수 있다(재리뷰 실데이터: 동부간선도로에서 2대).
- 롱컨트롤 또는 ICBM이 있으면 SCC-Map이 한국 모드에서 기본으로 켜진다(카메라 종류 토글 기본 ON). 그래서 인게이지 중에는 `SCC-M` 배지가 항상 보인다. SLA 모드가 Off면 속도 표지(앞쪽 표지 포함)가 숨겨지지만 카메라 감속은 그대로 동작한다. 화면에서 이를 알려 주는 것은 `SCC-M` 배지뿐이다. 릴리스 노트에 적는다.

## 실패 처리

- `kind` 없는 구 DB: 종류를 모르는 카메라는 비어 있지 않은 집합이면 통과한다. 그래서 개별 토글은 DB가 다시 만들어질 때까지 효과가 없다. 넷을 모두 끄는 것만 바로 적용된다. 열 때 경고 로그를 한 번 남긴다.
- API 분류 필드 누락: 기존 DB가 있으면 그대로 둔다. 카메라 DB가 아예 없으면 새로 만들고, 빠진 필드가 가리던 종류만 과속으로 떨어진다(예: `regltSe`가 없으면 신호·과속 카메라가 과속이 된다). `open_db`는 카메라 파일이 있어야 열리므로, 이때 거부하면 링크 제한속도까지 포함해 한국 지도 데이터를 전부 못 쓰게 된다(최종 리뷰 M1).
...
- **카메라를 차가 달리는 링크에 매칭** — 회랑 검사 대신 차의 현재 링크와 카메라를 맞추면 곡선 구간 카메라도 감속 대상으로 되살릴 수 있다. 경로 안내 중에는 경로선 기준으로 이미 해결됐고(2절), 남은 것은 경로 없이 달릴 때다. 별도 작업으로 한다.
