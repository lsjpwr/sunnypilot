...
- 진행선에서 옆으로 30 m보다 먼 카메라(옆 도로·반대 차로)는 HUD 앞쪽 표지에만 나오고 감속하지 않는다. 곡선 구간 카메라 일부도 같은 이유로 표지만 뜬다.
...
- API 분류 필드 누락: 기존 DB가 있으면 그대로 둔다. 카메라 DB가 아예 없으면 새로 만들고 전부 과속으로 분류한다. `open_db`는 카메라 파일이 있어야 열리므로, 이때 거부하면 링크 제한속도까지 포함해 한국 지도 데이터를 전부 못 쓰게 된다(최종 리뷰 M1).
